...
def show_data_slug(request, slug=None):
    show_in_html = True
    if slug is not None:
        try:
            model_row = FirstModel.objects.get(slug=slug)
        except:
            model_row = None
    context = {
        'model_row': model_row,
    ...
    return render(request, "firstapp/detail.html", context=context)
    ...
    if request.method == "POST":  # if we use line 58 & 60 instead of line 57 & 59 then we do not have to check for POST method in DjangoForm & DjangoModelForm
        ...
        if "django_form" in query_dict_keys:
            ...
            if modelform.is_valid():
                ...
                ## URLs Reverse other possible uses are below with redirect method
                # return redirect(firstapp_obj.get_absolute_url())
                return redirect("user-slug", slug=firstapp_obj.slug)
            # if modelform.is_valid():
